 ...
 *
 * match 명령어의 옵션 이름을 바꾸고 싶으면 여기만 수정한다.
 ...
 * match 명령어 옵션을 읽는다.
 * argv 예시:
 *   logscope match --spring samples/spring.log --nginx samples/nginx.log
 */
static int parse_match_args(int argc, char **argv, const char **spring_log_path, const char **nginx_access_log_path)
{
    int i;
...
/* match 명령어: 두 파일을 각각 분석한 뒤, 공통 기준으로 비교해서 출력한다. */
static int run_match_command(const char *spring_log_path, const char *nginx_access_log_path)
{
    ...
    print_match_log_summary(&spring_summary, &nginx_access_summary);
...
/*
 * 프로그램 진입점이다.
 * argv[1]에 들어온 명령어 이름을 보고 spring/nginx/match 중 하나로 분기한다.
 */
int main(int argc, char **argv)
{
    ...
    if (strcmp(argv[1], CMD_MATCH) == 0) {
        if (!parse_match_args(argc, argv, &spring_log_path, &nginx_access_log_path)) {
            print_usage(argv[0]);
            return 1;
        }

        return run_match_command(spring_log_path, nginx_access_log_path);
    }
