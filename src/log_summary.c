...
/* Spring ErrorCode까지 포함한 key다. 예: POST + /api/orders + 409 + BREAD_002 */
static int make_route_code_key(
    ...
    const char *code)
{
    return snprintf(dest, dest_size, "%s\t%s\t%d\t%s", method, path, status, code) > 0;
}
...
static void spring_log_summary_init(SpringLogSummary *summary)
{
    summary->total = 0;
    summary->skipped = 0;
    counter_init(&summary->events);
    counter_init(&summary->codes);
...
{
    counter_free(&summary->events);
    counter_free(&summary->codes);
...
static int add_spring_log_entry(SpringLogSummary *summary, const SpringLogEntry *entry)
{
    ...
    /* 숫자/여러 필드를 counter용 문자열 key로 변환한다. */
    ...
    if (!make_route_code_key(
            route_code_key,
            sizeof(route_code_key),
            entry->method,
            entry->path,
            entry->status,
            entry->code)) {
    ...
    /* 같은 로그 한 줄을 event, code, status, path, route_code 기준으로 각각 집계한다. */
    if (!counter_add(&summary->events, entry->event)) {
        return 0;
    }
    if (!counter_add(&summary->codes, entry->code)) {
        return 0;
    }
